...
interface MovieCardProps {
  ...
  isWatched?: boolean;  // ← ADD THIS: to indicate if the movie is watched for better UI feedback
}
...
const MovieCard: React.FC<MovieCardProps> = ({
  ...
  onAdd,
  isInWatchlist = false,
  isWatched = false,  // ← ADD THIS: default to false
}) => {
  ...
  const handleImageError = (
    ...
    if (parent && !parent.querySelector(".no-poster-fallback")) {
      ...
      fallback.className =
        "no-poster-fallback w-full h-full bg-gray-200 flex flex-col items-center justify-center";
      fallback.innerHTML = `
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="mt-auto flex flex-col gap-2">
          {isInWatchlist ? (
            onRemove ? (
              <>
                {/* Watchlist Page - Show full controls */}
                <div className="mb-3">
              ...
              // Search Page - movie is already watched
              <div className="bg-emerald-100 text-emerald-800 text-center py-1.5 px-3 rounded-lg font-medium text-sm flex items-center justify-center gap-1.5">
                <svg
              ...
              // Search Page - in watchlist but not watched
              <div className="bg-yellow-100 text-yellow-800 text-center py-1.5 px-3 rounded-lg font-medium text-sm">
                Added to Watchlist
              </div>
            )
          ) : (
